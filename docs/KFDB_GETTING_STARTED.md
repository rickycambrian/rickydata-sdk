...
If you are building a non-TypeScript client that implements HTTP calls directly, use the cross-language auth + KFDB wire contract at [`docs/contracts/cross-language-auth-kfdb.yaml`](contracts/cross-language-auth-kfdb.yaml). That artifact is the supported reference for wallet-token creation, canonical `mcpwt_` payload fields, and KFDB sign-to-derive session setup.
