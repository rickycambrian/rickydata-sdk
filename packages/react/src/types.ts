...
export interface ChatMessage {
  ...
  costUSD?: string;
  images?: ChatImage[];
  /** Model that actually served the turn (from the gateway's done event). */
  model?: string;
  /** How the gateway resolved the model ('explicit'|'session'|'routing'|'exploration'|'wallet_default'|'provider_default'|'fallback'). */
  modelSource?: string;
}
