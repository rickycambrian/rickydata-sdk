...
describe('AgentClient', () => {
  ...
  describe('chat', () => {
    ...
    it('unlocks a locked qwen key (the wallet-default provider after a gateway deploy relock)', async () => {
      const fetchSpy = vi.spyOn(globalThis, 'fetch');

      fetchSpy
        // Exact production body observed live after a TEE deploy wiped the session-key cache
        .mockResolvedValueOnce({
          ok: false,
          status: 400,
          text: () => Promise.resolve(JSON.stringify({
            error: 'missing_secrets',
            needsUnlock: true,
            message: 'Your Qwen Token Plan key is encrypted and locked. Unlock provider keys in Settings before chatting.',
            missingSecrets: [{ serverId: 'qwen', serverName: 'Qwen Token Plan', secretKeys: ['QWEN_API_KEY'] }],
          })),
        } as Response)
        .mockResolvedValueOnce({
          ok: true,
      ...
      const signMessage = vi.fn().mockResolvedValue('0xsigned');
      const client = new AgentClient({ token: 'jwt', signMessage, sessionStorePath: null });
      const result = await client.chat('test-agent', 'hi', { sessionId: 'sess-1' });
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ signature: '0xsigned', nonce: 'derive-nonce', providers: ['qwen'] }),
        }),
      );
    });
  ...
  describe('createSession', () => {
    it('omits the model key entirely when no model is given (gateway resolves)', async () => {
      const fetchSpy = vi.spyOn(globalThis, 'fetch');
      ...
      fetchSpy
        ...
        .mockResolvedValueOnce({
          ...
          json: () => Promise.resolve({
            ...
            agentId: 'test-agent',
            model: 'glm-5.1',
            modelSource: 'wallet_default',
            createdAt: '2026-08-09T00:00:00.000Z',
          }),
        } as Response);

      const client = new AgentClient({ privateKey: PRIVATE_KEY, sessionStorePath: null });
      const session = await client.createSession('test-agent');

      expect(JSON.parse(fetchSpy.mock.calls[2][1]!.body as string)).toEqual({});
      expect(session.model).toBe('glm-5.1');
      expect(session.modelSource).toBe('wallet_default');
    });

    it('still sends the model when one is explicitly given', async () => {
      const fetchSpy = vi.spyOn(globalThis, 'fetch');
      ...
      const client = new AgentClient({ privateKey: PRIVATE_KEY, sessionStorePath: null });
      await client.createSession('test-agent', 'sonnet');

      expect(JSON.parse(fetchSpy.mock.calls[2][1]!.body as string)).toEqual({ model: 'sonnet' });
    });
    ...
    it('passes through Qwen Code execution engine requests', async () => {
      const fetchSpy = vi.spyOn(globalThis, 'fetch');
      ...
      fetchSpy
        ...
        .mockResolvedValueOnce({
          ...
          json: () => Promise.resolve({
            ...
            executionEngine: 'qwen-code',
          }),
        } as Response);

      const client = new AgentClient({ privateKey: PRIVATE_KEY, sessionStorePath: null });
      const session = await client.createSession('test-agent', 'qwen3.8-max', 'qwen-code');

      expect(session.executionEngine).toBe('qwen-code');
      expect(JSON.parse(fetchSpy.mock.calls[2][1]!.body as string)).toEqual({
        model: 'qwen3.8-max',
        executionEngine: 'qwen-code',
      });
