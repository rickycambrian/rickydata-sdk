...
describe('buildClaudeCodeHookTraceOperations pass-through fields', () => {
  ...
  it('does not alter existing emitted properties when pass-through fields are absent', () => {
    ...
    // 4 since ClaudeCodeTurn/ClaudeCodeToolUse started carrying embeddable text.
    // The session node itself is unchanged; it shares the builder's one constant.
    expect(props.schema_version).toEqual({ Integer: 4 });
