...
describe('wallet commands', () => {
  ...
  describe('wallet settings set', () => {
    ...
    it('allows evidence-routing settings keys', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({}),
        text: async () => '',
      ...
      await program.parseAsync([
        'node', 'rickydata', 'wallet', 'settings', 'set', 'routingMode', 'advisory',
      ]);
      await program.parseAsync([
        'node', 'rickydata', 'wallet', 'settings', 'set', 'routingApprovedConfig', 'claude-haiku',
      ]);

      expect(mockFetch.mock.calls.map((call) => call[1].body)).toEqual([
        JSON.stringify({ routingMode: 'advisory' }),
        JSON.stringify({ routingApprovedConfig: 'claude-haiku' }),
      ]);
    });
