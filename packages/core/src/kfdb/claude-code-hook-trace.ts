...
// v4 adds embeddable text: `text` on ClaudeCodeTurn, `input_text`/`output_text`
// on ClaudeCodeToolUse. v3 rows carry only contentHash/contentLength, so the
// version is what tells a reader whether absent text means "not recorded" or
// "this turn genuinely had none".
...
/**
 * Cap on text written to a node property, matching MAX_EMBED_CHARS in
 * kfdb-api `file_embeddings/handlers.rs`. Past this the embedder never reads it,
 * so the extra bytes buy payload and nothing else.
 *
 * ponytail: one vector per node — the tail of a 250KB tool response is not
 * searchable. Chunk into child nodes if recall on that tail turns out to matter.
 */
const MAX_TEXT_CHARS = 30_000;

/** Tool payloads as embeddable text. Objects are stably serialised so the same
 *  payload always yields the same string (and so the same content hash). */
...
/**
 * The turn as prose: each prompt and each final assistant message, in order.
 *
 * ClaudeCodeTurn carried no text property at all, so a turn was unreachable by
 * semantic search even though every word of it was already being written to the
 * graph's KV store as a content artifact.
 */
function turnText(trace: ClaudeCodeHookTrace): string | undefined {
  const parts: string[] = [];
  for (const event of trace.events) {
    for (const candidate of [event.prompt, event.lastAssistantMessage]) {
      const text = typeof candidate === 'string' ? candidate.trim() : '';
      if (text) parts.push(text);
    }
  }
  return parts.length ? parts.join('\n\n').slice(0, MAX_TEXT_CHARS) : undefined;
}
...
export function buildClaudeCodeHookTraceWriteBundle(trace: ClaudeCodeHookTrace): ClaudeCodeHookTraceWriteBundle {
  ...
  const operations: Array<Record<string, unknown>> = [
    ...
    { operation: 'create_node', id: turnNodeId, label: 'ClaudeCodeTurn', mode: 'merge', properties: { agent_id: value(trace.agentId), session_id: value(trace.sessionId), claude_session_id: value(trace.claudeSessionId), turn_index: value(trace.turnIndex), model: value(model), provider: value('anthropic'), execution_engine: value('claude-code'), cwd: value(trace.cwd ?? ''), started_at: value(trace.startedAt), completed_at: value(trace.completedAt), event_count: value(trace.events.length), text: value(turnText(trace)), schema_version: value(TRACE_SCHEMA_VERSION), repository: value(trace.repository), base_repository: value(trace.baseRepository), result_repository: value(trace.resultRepository), work_contract: value(trace.workContract), source_intent_ref: value(trace.sourceIntentRef) } },
