...
export async function startChatRepl(opts: ChatReplOptions): Promise<void> {
  ...
  // undefined = gateway resolves from wallet settings / evidence routing
  let currentModel: string | undefined = opts.model;

  // Print banner
  console.log(chalk.dim('─'.repeat(50)));
  console.log(`${chalk.bold('Agent:')} ${chalk.cyan(opts.agentId)}`);
  console.log(`${chalk.bold('Model:')} ${chalk.cyan(currentModel ?? 'auto (gateway resolves)')}`);
  if (currentSessionId) {
  ...
  const prompt = () => {
    ...
    rl.question(chalk.blue('you> '), async (input) => {
        ...
        switch (cmd) {
          ...
          case 'model':
            if (args[0]) {
              ...
              console.log(`Current model: ${chalk.cyan(currentModel ?? 'auto (gateway resolves)')}`);
            }
            break;
      ...
      try {
        ...
        // Surface which model actually served the turn
        if (result.model) {
          console.log(chalk.dim(`model: ${result.model}${result.modelSource ? ` (${result.modelSource})` : ''}`));
        }
