...
Cross-language clients that implement raw HTTP calls directly should mirror the wire contract in [`docs/contracts/cross-language-auth-kfdb.yaml`](docs/contracts/cross-language-auth-kfdb.yaml) rather than depend on the TypeScript SDK at runtime.
...
For non-TS clients that need wallet-token auth or KFDB sign-to-derive session setup, use the shared contract artifact at [`docs/contracts/cross-language-auth-kfdb.yaml`](docs/contracts/cross-language-auth-kfdb.yaml). It documents the stable HTTP shapes without requiring JS storage, CLI UX, or runtime coupling.
...
Source: [`rickydata-sdk`](https://github.com/rickycambrian/rickydata-sdk)
